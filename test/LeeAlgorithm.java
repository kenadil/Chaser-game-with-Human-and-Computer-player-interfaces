...
private final int matrixWidth = 8, matrixHeight = 8;
...
private ArrayList<Node> findPath(Node start, Node goal) {
    ...
    for (int i = 1; i < MAXITERATIONS; i++) {
        ...
        nodeList = markNeighbors(nodeList, i+1);
...
private ArrayList<Node> markNeighbors(ArrayList<Node> neighborList, int iteration) {
	System.out.println("Proceed to marking");
...
private ArrayList<Node> backtraceFromGoal(Node fromGoal, Node toStart) {
	System.out.println("Proceed to path creation");
...
private Node getNeighbor(Node node) {
	//System.out.println("Proceed to get neighbor to path");
...
public static void main(String[] args) {
    LeeAlgorithm l = new LeeAlgorithm(new Map(args[0]));
    ArrayList<Node> output = l.findPath(new Node(2, 5), new Node(2, 0));
